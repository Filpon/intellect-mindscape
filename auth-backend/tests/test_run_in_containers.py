...
async def test_health_check(backend_container_runner):
    ...
    The test sends GET request to the `/check-auth` endpoint
    ...
    """
    response = await backend_container_runner.get(url="/check-auth", timeout=10)
...
async def test_generate_auth(backend_container_runner):
    ...
    The test sends GET request to the `/api-auth/v1/auth/generate-auth` endpoint
    ...
    run the backend container and interact with it during tests
    """
    response = await backend_container_runner.get(url="/api-auth/v1/auth/generate-auth")
...
async def test_auth_generate_calback(backend_container_runner):
    ...
    This test sends GET request to the `/api-auth/v1/auth/callback` endpoint
    with a provided authorization code and asserts that the response status
    code indicates an error, either due to a bad request or unauthorized access

    :param backend_container_runner: Fixture that provides way to
    run the backend container and interact with it during tests
    """
    response = await backend_container_runner.get(
        url="/api-auth/v1/auth/callback?code=auth_code"
    )
    assert response.status_code in {
...
async def test_register_user(backend_container_runner):
    ...
    The test sends POST request to the `/api-auth/v1/auth/register` endpoint
...
async def test_login(backend_container_runner):
    ...
    The test first sends POST request to the `/api-auth/v1/auth/register`
    endpoint to create a new user with a generated username and password.
    It then sends POST request to the `/api-auth/v1/auth/token` endpoint
...
async def test_common_user_refresh_token(backend_container_runner, common_user_tokens):
    """
    Testing the token refresh functionality for common users.

    The test sends POST request to the `/api-auth/v1/auth/refresh`
...
async def test_admin_user_refresh_token(backend_container_runner, admin_user_tokens):
    """
    Testing the token refresh functionality for admin users.
...
async def test_common_user_logout(backend_container_runner, admin_user_tokens):
    """
    Testing the logout functionality for common users.

    The test sends POST request to the `/api-auth/v1/auth/logout`
    endpoint with the refresh token of a common user. It asserts that
...
async def test_admin_user_logout(backend_container_runner, admin_user_tokens):
    """
    Testing the logout functionality for admin users.
...
async def test_fetch_all_users(backend_container_runner, admin_user_tokens):
    """
    Testing the availability of the backend service's user fetching endpoint.

    The test sends GET request to the `/api-auth/v1/auth/users` endpoint
...
async def test_update_user_account(backend_container_runner, admin_user_tokens):
    ...
    that the new password works by attempting to obtain a token
    ...
    response_update_user = await backend_container_runner.put(
        url=f"/api-auth/v1/auth/users/{user_sub_id}",
        headers={"Authorization": f"Bearer {admin_user_tokens['access_token']}"},
...
async def test_common_user_protected_route(
    ...
    The test sends GET request to the `/api-auth/v1/auth/protected` endpoint
    ...
    """
    response = await backend_container_runner.get(
        url="/api-auth/v1/auth/protected",
...
async def test_common_user_introspect_token(
    ...
    The test sends POST request to the `/api-auth/v1/auth/introspect` endpoint
    using the access token of common user. It asserts that the response status
    ...
    """
    response = await backend_container_runner.post(
        url="/api-auth/v1/auth/introspect",
        headers={"Authorization": f"Bearer {common_user_tokens['access_token']}"},
    )
    assert response.status_code == status.HTTP_200_OK
