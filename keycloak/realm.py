...
def replace_placeholders(config: dict):
    ...
    placeholders in t2he format '${key}' within the corresponding dictionary entries
        ...
        if isinstance(value, dict):
            ...
            if value.startswith("${") and value.endswith("}"):
                ...
                config[key] = os.getenv(
                    env_var_name, f"{env_var_name}"  # value
...
def main():
    ...
    with open("realm-config-initial.json", "r", encoding="utf-8") as f:  # /tmp/import/realm.json
        config = json.load(f)

    replace_placeholders(config=config)
    # Save the modified configuration back to a new JSON file
    with open(
        "realm-config.json", "w", encoding="utf-8"
    ) as f:  # /tmp/import/realm-mod.json
        json.dump(config, f, indent=2)

    print("Configuration was saved to realm-config.json")
